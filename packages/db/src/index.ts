...
// instance*. That default is computed from the machine Prisma is running on
// and knows nothing about how many instances share the database. Once the API
// runs clustered — one worker per core, each with its own client — an 8-core
// container silently asks for 8 x 17 = 136 connections. Supabase's pooler
// caps well below that, so the surplus workers get "too many clients"
// errors under exactly the load the clustering was added to survive.
// =============================================================================
...
/**
 * Applies pool sizing to the connection string unless it is already spelled
 * out there. Editing the URL is how Prisma is configured for this — there is
 * no programmatic pool option.
 *
...
function buildDatabaseUrl(): string | undefined {
  ...
  try {
    ...
    if (!url.searchParams.has("pool_timeout")) {
      ...
      // Failing fast is better than queueing forever behind a stuck query:
      // a request that has already waited 10s is a request the user gave up on.
...
function createClient() {
  ...
  // with it, the offending query and its duration are in the logs by the time
  // anyone asks.
  ...
  client.$on("error", (event) => {
    console.error(JSON.stringify({ level: "error", msg: "prisma error", target: event.target, detail: event.message }));
  });

  client.$on("warn", (event) => {
    console.warn(JSON.stringify({ level: "warn", msg: "prisma warning", target: event.target, detail: event.message }));
  });
