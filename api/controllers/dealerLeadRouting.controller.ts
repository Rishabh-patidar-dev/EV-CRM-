...
// ============================================================================
// Implements Luxus Green's "leads passed to the nearest authorised dealer"
// model. A retail Lead (from the Leads module / landing page) is matched to a
// dealer by territory and handed over for follow-up.
// ============================================================================
    ...
    try {
      ...
      if (!dealerId) {
        dealerId = await findNearestDealer({
          ...
          district: (lead as any).district ?? (lead as any).city,
        });
      }
