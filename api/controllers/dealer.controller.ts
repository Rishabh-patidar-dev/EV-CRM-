...
export class DealerController {
  ...
  async list(req: Request, res: Response) {
    try {
      ...
      if (state) where.state = { equals: state as string, mode: "insensitive" };
  ...
  async stats(_req: Request, res: Response) {
    try {
      const [byStatus, byTier, byState, totalDealers, openFinance, openService] =
        await Promise.all([
  ...
  async getById(req: Request, res: Response) {
    try {
      ...
      res.json({ ...dealer, currentAttainment: attainment });
    } catch (error) {
  ...
  async createFromApplication(req: Request, res: Response) {
    try {
      ...
      const state = (app as any).state ?? req.body?.state ?? "";
      if (!state) {
        return handleValidationError(
          res,
      ...
      const dealer = await prisma.dealer.create({
        data: {
          legalName: (app as any).legalName ?? (app as any).businessName ?? "Unnamed dealer",
          tradeName: (app as any).tradeName ?? null,
          dealerCode,
          gstNumber: (app as any).gstNumber ?? null,
          panNumber: (app as any).panNumber ?? null,
          principalName: (app as any).contactName ?? (app as any).applicantName ?? "—",
          phone: (app as any).phone ?? "—",
          email: (app as any).email ?? null,
          city: (app as any).city ?? null,
          district: (app as any).district ?? null,
          state,
          pincode: (app as any).pincode ?? null,
          tier: (app as any).tier ?? "STANDARD",
          status: "ACTIVE",
  ...
  async update(req: Request, res: Response) {
    try {
      ...
      if (Array.isArray(b.segments)) data.segments = b.segments;
  ...
  async setTerritories(req: Request, res: Response) {
    try {
      ...
      await prisma.$transaction([
        ...
        prisma.dealerTerritory.createMany({
          data: territories
