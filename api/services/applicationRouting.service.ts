...
//  Domain logic for the dual-intent landing-page ingestion.
//   - normalises + validates incoming payloads (reuses existing helpers)
//   - extracts UTM attribution
//   - seeds the per-stage document checklist from ONBOARDING_DOC_CATALOG
//  Place in: apps/api/src/services/applicationRouting.service.ts
// =============================================================================
...
export const ONBOARDING_DOC_CATALOG: Record<OnboardingStage, DocSpec[]> = {
  ...
  LEGAL_AGREEMENT: [
    { docKey: "LOI_SIGNED", label: "Signed Letter of Intent (LOI)" },
  ...
  FACILITY_BRANDING: [
    { docKey: "DISCOM_LOAD_SANCTION", label: "DISCOM power load sanction letter (EV fast-charging capable)" },
    { docKey: "FITOUT_SIGNOFF", label: "Architectural fit-out sign-off by OEM branding team" },
    { docKey: "PCB_NOC", label: "State Pollution Control Board CTE/CTO for wash/service bays" },
    { docKey: "BRANDING_CERTS", label: "Branding execution & structural stability certificates" },
  ],
  STAFF_TRAINING: [
    { docKey: "EMPLOYEE_KYC", label: "Core management team KYC + credentials" },
    { docKey: "HV_SAFETY_CERTS", label: "High-voltage / Li-ion safety certifications for technicians" },
    { docKey: "LMS_PROGRESS", label: "LMS completion certificates (sales + CRM navigation)" },
  ],
  GO_LIVE: [
    { docKey: "RTO_TRADE_CERT", label: "RTO Trade Certificate (Form 16, CMVR)" },
    { docKey: "COMMERCIAL_REG", label: "Shops & Est. license + Municipal Trade license" },
    { docKey: "INSURANCE_POLICIES", label: "Blanket asset, public liability & workmen's comp insurance" },
  ],
  OPERATIONAL: [],
};

/** Ordered stage list — used to validate forward-only transitions. */
