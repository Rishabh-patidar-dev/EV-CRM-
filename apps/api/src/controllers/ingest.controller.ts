...
//
//  Copied from the delivered bundle at /api/controllers/ingest.controller.ts
//  with no logic changes — this file has no Postgres-specific query syntax.
// =============================================================================
...
export class IngestController {
  ...
  private async routeRetailInquiry(
    ...
    const companyName = f.company || f.company_name || null;
    const city = f.city || null;
    const state = f.state || null;
    const pincode = f.pincode || f.pin_code || null;

    // Real innocrm-staging scoring: completeness x0.7 + quality x0.3 across
    // 7 fields (see leadScoring.service.ts) — run on every fresh lead so the
    // sales desk can triage by score, not just recency.
    const scored = new LeadScoringService().calculateLeadScore({
